      ...
      <div class="row justify-content-around">
        <form action="" class="col-md-8 bg-light p-3 my-3" method="POST" >
          ...
          <div class="form-group">
            ...
            <input type="text" name ="icon" id="icon" class="form-control" required value="{{old('icon')}}">
            @error('icon')
            <span style="color: red;">{{$message}}</span>
            @enderror
          </div>
          ...
          <button type="submit" name="submit" class="btn-primary btn mb-3">Thêm phong cách</button>
        </form>
      </div>
    </div>
  </div>
